...
- **`wifi_monitor.rs`** — `WifiMonitor` for the `--trusted-wifi` SSID whitelist. Modern macOS redacts the SSID from unprivileged sources (SCDynamicStore `SSID_STR` is empty, `ipconfig` prints `<redacted>`), so detection and retrieval are split: an SCDynamicStore pattern watch on `State:/Network/Interface/.*/AirPort` (CFRunLoop thread) detects changes, and a separate query thread debounces 500ms then resolves the SSID via `system_profiler SPAirPortDataType -json` (~2s; never on a hot path — consumers only receive resolved `WifiEvent`s over a channel; do not use `-detailLevel mini`, it intermittently omits the current-network section). Query failure emits `ssid: None` = untrusted, failing open toward establishing the VPN. On a whitelisted SSID the controller suspends (`SuspendedOnTrustedWifi`): LCP Terminate + full local teardown (routes, DNS, utun) while keeping SVPNCOOKIE; leaving the whitelist reconnects without re-auth.
